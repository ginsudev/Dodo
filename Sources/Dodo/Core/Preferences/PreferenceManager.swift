...
final class PreferenceManager {
    ...
    func loadSettings(withDictionary dict: [String: Any]) {
        self.settings = .init(withDictionary: dict)
    }
}
...
struct Settings {
    ...
    init(withDictionary dict: [String: Any]) {
        ...
        if let timeMediaPlayerStyle = TimeMediaPlayerStyle(rawValue: dict["timeMediaPlayerStyle"] as? Int ?? 2),
           let playerStyle = MediaPlayerStyle(rawValue: dict["playerStyle"] as? Int ?? 0) {
            mediaPlayer = .init(
                timeMediaPlayerStyle: timeMediaPlayerStyle,
        ...
        if let fontType = FontType(rawValue: dict["selectedFont"] as? Int ?? 0) {
            appearance = .init(
        ...
        if let gridSizeType = GridSizeType(rawValue: dict["favouriteAppsGridSizeType"] as? Int ?? 0) {
            favouriteApps = .init(
                ...
                favouriteAppsGridSizeType: gridSizeType,
        ...
        let indicatorSize = dict["indicatorSize"] as? Double ?? 18.0
        statusItems = .init(
            hasStatusItems: dict["hasStatusItems"] as? Bool ?? true,
            statusItemSize: indicatorSize,
